"""
Morphine Response Outcome Extraction Using DuckDB
...
class MorphineResponseExtractor:
    """Extract morphine-induced locomotor response using DuckDB"""
    ...
    def extract_post_dose_activity(self, cage_id, replicate, dose_number=1, window_minutes=60):
        """Extract post-dose locomotor activity"""
        ...
        end_time = dose_time + timedelta(minutes=window_minutes)
        ...
        FROM read_parquet('{self.s3_base}/cage_id={cage_id}/date={dose_date}/animal_aggs_short_id.parquet')
        WHERE date = '{dose_date}'
            AND name = 'animal.distance_travelled'
            AND resolution = 60
            AND CAST(time AS TIMESTAMP) >= CAST('{dose_time_str}' AS TIMESTAMP)
    ...
    def calculate_response_score(self, baseline, post_dose, method='percent_change_mean'):
        ...
        Methods:
        - percent_change_mean: (post_mean - baseline_mean) / baseline_mean * 100
        - percent_change_peak: (post_peak - baseline_95pct) / baseline_95pct * 100
        - z_score: (post_mean - baseline_mean) / baseline_std
        - auc_normalized: post_auc / baseline_mean
        - absolute_change: post_mean - baseline_mean
        """
    ...
    def extract_response_for_cage(self, cage_id, replicate, dose_number=1, 
        ...
        # Extract post-dose
        post_dose = self.extract_post_dose_activity(cage_id, replicate, dose_number, window_minutes)
        print(f" | Post-dose: {post_dose['post_dose_mean']:.2f} cm/s", end='', flush=True)
        ...
        result = {
            ...
            'response_window_minutes': window_minutes
        }
    ...
    def extract_all_responses(self, dose_number=1, response_method='percent_change_mean',
                             window_minutes=60, morphine_only=True):
        ...
        for replicate_name, replicate_info in self.dose_times.items():
            ...
            print(f" Response window: 0-{window_minutes} minutes post-injection")
            ...
            for cage_id in replicate_info['cages']:
                ...
                try:
                    response = self.extract_response_for_cage(
                        ...
                        response_method, window_minutes
                    )
...
def main():
    ...
    print("="*70)
    print(" MORPHINE RESPONSE OUTCOME EXTRACTION")
    print("="*70)
    ...
    print("\nExtracting morphine responses...")
    
    # Extract responses for first dose
    ...
    # Save results
    output_file = 'morphine_response.csv'
